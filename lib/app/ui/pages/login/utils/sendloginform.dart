...
Future<void> sendLoginForm1(BuildContext context) async {
  final controller1 = LoginProvider1.read;
  final isValidForm1 = controller1.formKey.currentState!.validate();
  if (isValidForm1) {
    ProgressDialog.show(context);
    final response = await controller1.submmit();
    router.pop();
    if (response.error != null) {
      String errorMessage = "";
      switch (response.error) {
